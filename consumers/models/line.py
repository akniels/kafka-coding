...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == "com.udacity.journey.stations": # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic() == "com.udacity.journey.arrivals": # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif message.topic() == "com.udacity.journey.turnstile.summary": # Set the conditional to the KSQL Turnstile Summary Topic
